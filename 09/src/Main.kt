...
fun part01(arg: IntArray): Long {
    ...
    input.forEachIndexed { index, i ->
        ...
        else
        {
            ...
            {
                sum += freeMemory[index]!!.map<Pair<Int, Int>, Long> {
                    val s = (0..<it.second).sumOf { j -> (j.toLong() + position) * it.first }
                    position += it.second
                    return@map s
                }.sum()
            }
...
fun part02(arg: IntArray): Long {
    ...
    input.forEachIndexed { index, i ->
        ...
        else
        {
            ...
            {
                sum += freeMemory[index]!!.map<Pair<Int, Int>, Long> {
                    val s = (0..<it.second).sumOf { j -> (j.toLong() + position) * it.first }.toLong()
                    position += it.second
                    return@map s
                }.sum()
